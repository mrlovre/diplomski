...
def decode_pair(c, N, inv=False):
    b1 = N + 0.5
    b2 = (N - 0.5) ** 2
    return __decode_pair(c, N, b1, b2, inv)

def decode_pairs(cs, N, inv=False):
    b1 = N + 0.5
    b2 = (N - 0.5) ** 2
    return np.array(np.vectorize(lambda c: __decode_pair(c, N, b1, b2, inv))(cs)).transpose()

def __decode_pair(c, N, b1, b2, inv):
        ...
        if return_weights:
            weights = np.abs(np.hstack(((diffs[ts_invs, invs] - means[ts_invs - T + 1, invs])
                                       ...
                                       (diffs[ts_no_invs, no_invs] - means[ts_no_invs - T + 1, no_invs])
                                        / np.sqrt(varss[ts_no_invs - T + 1, no_invs])))[ts_order, ...])
