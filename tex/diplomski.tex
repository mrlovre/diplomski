...
\chapter{Metode i koncepti}
  ...
  \end{equation}
  Oznake $\Efromto{P_X^{(\tau)}}{t_1 \le \tau < t_2}$ i $\Varfromto{P_X^{(\tau)}}{t_1 \le \tau < t_2}$ predstavljaju očekivanje i varijancu cijene vrijednosnice $X$ nad intervalom $\q[t_1, t_2\w>$.
  ...
  \begin{figure}[H]
    ...
    \includegraphics[width=1.0\linewidth]{graphics/diff-norm.png}
    \caption{
      Procijenjene rezidualne vrijednosti $\hat{\varepsilon}\q[t\w]$.}
    \label{fig:residual}
  \end{figure}
...
%  In addition to ordering of the entities, utility function also provides the intensity of preference for a particular entity, hence it is more informative when it comes to the decision making.
  Funkcija korisnosti \engl{utility function} $U\colon \Omega \to \mathbb{R}$ je preslikavanje iz skupa dobara u skup realnih brojeva, na način da poredak preslikanih realnih brojeva odgovara poretku dobara prema individualnoj preferenciji, tj. vrijedi $\forall x, y \in \Omega \colon x \succ y \Leftrightarrow U\q(x\w) > U\q(y\w)$.
...
%  A detailed description of used statistical measures the procedure is given later in \ref{sub:creating-graph}.
  ...
  Graf preferencije toga je pomoćna struktura koja služi pri prevođenju toka preferencije među čvorovima u individualne mjere preferencije svakog čvora.
  Ako se zanemare težine bridova u grafu i ostave samo smjerovi, iz njega se direktno može pročitati relacija preferencije.
  Težine u grafu daju dodatnu informaciju o funkciji korisnosti koja odgovara toj relaciji preferencije, za razliku od situacije kada težina nema, što je ekvivalentno tome da su sve preferencije jednakog intenziteta.
...
%  Instead of aiming at a consistent preference relation, we devise a consistency measure that describes similarity between the original graph and its nearest consistent reconstruction, and use it as an additional parameter in decision making.
  ...
  Prethodno spomenuta svojstva irrefleksivnosti i asimetričnosti uvijek vrijede i u grafu toka preferencije:
...
%  Equation (\ref{eq:flow}) determines $\matr{\phi^*}$ up to a constant (i.e. solution has one degree of freedom), so the following constraint is also included:
  ...
  \end{itemize}
  Kako je graf $\graph{G}$ potpun, tj. broj susjeda svakog čvora jednak je $N - 1$, izraz $\q[ \matr{B}^\T \matr{B} + \matr{J} \w]^{-1}$ se može pojednostavniti u $\frac{1}{N} \matr{I}$; i stoga, se (\ref{eq:final}) može svesti na:
  \begin{equation}
  \label{eq:final2}
...
%  It is now possible to define a consistency measure $\kappa$ as follows:
  ...
  Za konzistentan graf dobiven na ovaj način također vrijedi i da je suma preferencija po nekoj petlji jednaka 0.
...
%  $\kappa$ ranges from 0 to 1, with 0 meaning full inconsistency (virtually unreachable), and 1 meaning full consistency.
  ...
  Taj graf nije konzistentan: prema njemu $B$ nije izravno usporediva sa $C$, ali kako je $B \succ A$ i $A \succ C$, po tranzitivnosti bi trebalo biti i $B \succ C$, što u grafu nije slučaj.  
  \begin{figure}[h]
    \centering
...
%  On that note, $c_{i,j}^{\q(t\w)}$, $m_{i,j}^{\q(t\w)}$, and $d_{i,j}^{\q(t\w)}$ may be more efficiently stored if stored contiguously in memory as a matrix, using following coding scheme: a pair $(i, j)$, where $i < j$, should be encoded to $k$ as:
  ...
  Što se tiče redukcije memorijske potrošnje, $c_{i,j}^{\q(t\w)}$, $m_{i,j}^{\q(t\w)}$, i $d_{i,j}^{\q(t\w)}$ su idejno tenzori dimenzija $(D - T) \times N \times N$.
  ...
  Imajući to u vidu, ti tenzori se mogu gusto upakirati u matrice dimenzija $(D - T) \times \q. N\cdot(N - 1) \middle/ 2\w.$, koristeći sljedeće preslikavanje --- za kodiranje para $(i,j)$ u kod $k$ koristi se izraz:
...
%  An illustration is shown on Fig. \ref{fig:devmag}.
  ...
  \begin{equation}
  \label{eq:thresh}
  \q| c_{i,j}^{\q(t\w)} - m_{i,j}^{\q(t\w)} \w| > \alpha \cdot d_{i,j}^{\q(t\w)},
  \end{equation}
...
%  Portfolio diversification might be seen as a strategy for protection from fundamental risks, e.g. risk of asset default.
  Neka je $\matr{\phi}^{\q(t\w)} = \begin{bmatrix} \phi_1^{\q(t\w)} & \phi_2^{\q(t\w)} & \ldots & \phi_N^{\q(t\w)} \end{bmatrix}$ vektor individualnih preferenci vrijednosnica u vremenskom koraku $t$, te $\phi_i^{\q(t\w)}$ individualna preferencija vrijednosnice $i$ u vremenskom koraku $t$.
...
%  For determining the assets that should be held in the portfolio at time step $t$, we find such assets $i$ for which holds:
  ...
  \begin{equation*}
  \phi_i^{\q(t\w)} \ge \kappa^\prime \cdot \Phi,
  \end{equation*}
  gdje je $\Phi = \max_j \q\{ \q| \phi_j \w| \w\}$.
  Isto tako, za prodaju se biraju vrijednosnice $i$, za koje u trenutku $t$ vrijedi:
  \begin{equation*}
  \phi_i^{\q(t\w)} \le -\kappa^\prime \cdot \Phi.
  \end{equation*}
...
%These findings suggest that the proposed method produces consistent returns and may be feasible in a live market setting.
  ...
  Momenti se računaju u ukupno $D - T + 1$ vremenskih koraka, svaki put preko prozora veličine $T$, tako da je vremenska složenost nerekurzivne metode računanja momenata $\mathcal{O}\q(\q(D - T + 1\w)\cdot T\w)$, odnosno približno $\mathcal{O}\q(DT\w)$, kako je $D \gg T$.
  ...
  \end{align}
  Sada se u (\ref{eq:varalt}) $\Efromto{a^2\q[\tau\w]}{t - T < \tau \le t}$ također može izraziti preko nenormiranih momenata kao $\q. S_2^{(t)} \middle/ S_0^{(t)} \w.$,
  i $\Esqfromto{a\q[\tau\w]}{t - T < \tau \le t}$ kao $\q. \q(S_1^{(t)}\w)^2 \middle/ \q(S_0^{(t)}\w)^2 \w.$.
  ...
  Inicijalno je potrebno izračunati momente $S_0^{(T - 1)}$, $S_1^{(T - 1)} $ i $S_2^{(T - 1)}$, te iz njih $\Efromto{a\q[\tau\w]}{0 \le \tau < T}$ i $\Varfromto{a\q[\tau\w]}{0 \le \tau < T}$; u složenosti $\mathcal{O}\q(T\w)$.
  Nadalje, za svaki sljedeći pomak vremenskog prozora rekurzivno se ažuriraju vrijednosti nenormiranih momenata koristeći (\ref{eq:rec}), te se putem njih izračunavaju očekivanje i varijanca koristeći (\ref{eq:recmean}) i (\ref{eq:recvar}), u složenosti $\mathcal{O}\q(T - D\w)$.
  Ukupna vremenska složenost algoritma time je pala s $\mathcal{O}\q(DT\w)$ na $\mathcal{O}\q(D\w)$.
