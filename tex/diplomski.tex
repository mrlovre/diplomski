...
\newcommand{\Esq}[1]{\operatorname{\mathbf{E}^2}\left[#1\right]}
\newcommand{\Var}[1]{\operatorname{\mathbf{Var}}\left[#1\right]}
\newcommand{\Efromto}[2]{\operatorname{\mathbf{E}}\left[#1\, \middle\vert\, #2\right]}
\newcommand{\Esqfromto}[2]{\operatorname{\mathbf{E}^2}\left[#1\, \middle\vert\, #2\right]}
...
\title{Harry Potter\\ i analiza vremenskih nizova zasnovana na kompleksnim mrežama}
...
\chapter{Metode i koncepti}
  ...
  \label{eq:statarb}
  V^{(0)} = 0\\
  \quad \prob{V^{\left(t\right)} \ge 0} = 1,
  \end{gathered}
  \end{equation}
  ...
  Ipak, uz pretpostavku dovoljne likvidnosti tržišta, kako $t \to +\infty$, tako vjerojatnost $\prob{V^{\left(t\right)} > 0} \to 1$.
  ...
  \begin{figure}[p]
    ...
    \caption{
      ...
      Svjetlijim nijansama tih boja prikazano je punom linijom očekivanje, i crtkanom linijom udaljenost za jednu devijaciju od očekivanja; očekivanje i devijacija izračunati su na proteklom vremenskom periodu $T$ duljine 120 dana.
    }
    \label{fig:ab-prices}
  \end{figure}
  ...
  jednadžba (\ref{eq:stat1}) tvrdi da je za stacionaran proces $U\left[t\right]$ očekivanje slučajnog procesa $\E{U\left[t\right]}$ neovisna o vremenu $t$, a jednadžba (\ref{eq:stat2}) tvrdi da njegova autokorelacija $\R{UU}{t, \tau}$ ovisi samo o vremenskom pomaku $\tau$, ali ne i o $t$.
  ...
  \end{equation}
  gdje je $\mu = \E{U\left[t\right]}$, očekivanje, a $\varepsilon\left[t\right]$ rezidualna vrijednost za koju vrijedi $\E{\varepsilon\left[t\right]} = 0$.
  ...
  \begin{equation}
  \hat{\mu}, \hat{\beta} = \argmin_{\mu, \beta} \left\{\sum_{\tau = t - T}^{t - 1} \left\lvert P_X\left[\tau\right] + \beta P_Y\left[\tau\right] -\mu \right\rvert^2\right\},
  \end{equation}
  ...
  \end{equation}
  U drugom koraku provjerava se je li $\hat{\varepsilon}\left[t\right]$ zaista stacionaran proces, čime se dobiva p-vrijednost kao mjera pouzdanosti utvrđene kointegracije.
  ...
  \begin{figure}[H]
    ...
    \includegraphics[width=1.0\linewidth]{graphics/diff-norm.png}
    \caption{
      Procijenjene rezidualne vrijednosti $\hat{\varepsilon}\left[t\right]$.}
    \label{fig:residual}
  \end{figure}
...
%  and rolling means $m_{i,j}^{\left(t\right)}$ and standard deviations $d_{i,j}^{\left(t\right)}$ of log price differences over the past time window of size $T$ are obtained as follows:
  ...
  Logaritamske cijene $b_i^{\left(t\right)}$, i razlike logaritamskih cijena po parovima $c_{i,j}^{\left(t\right)}$ dobivaju se na sljedeći način:
  \begin{equation} b_i^{\left(t\right)} = \log\left(a_i^{\left(t\right)}\right) \end{equation}
  \begin{equation} c_{i,j}^{\left(t\right)} = b_i^{\left(t\right)} - b_j^{\left(t\right)}, \end{equation}
  i očekivanje $m_{i,j}^{\left(t\right)}$ i standardna devijacija $d_{i,j}^{\left(t\right)}$ razlika logaritamskih cijena tijekom proteklog perioda duljine $T$ dobivaju se kako slijedi:
  \begin{equation}
  \label{eq:mean}
...
%  Portfolio diversification might be seen as a strategy for protection from fundamental risks, e.g. risk of asset default.
  Neka je $\matr{\phi}^{\left(t\right)} = \begin{bmatrix} \phi_1^{\left(t\right)} & \phi_2^{\left(t\right)} & \ldots & \phi_N^{\left(t\right)} \end{bmatrix}$ vektor individualnih preferenci vrijednosnica u vremenskom koraku $t$, te $\phi_i^{\left(t\right)}$ individualna preferencija vrijednosnice $i$ u vremenskom koraku $t$.
...
%These findings suggest that the proposed method produces consistent returns and may be feasible in a live market setting.
  ...
  Momenti se računaju u ukupno $D - T + 1$ vremenskih koraka, svaki put preko prozora veličine $T$, tako da je vremenska složenost nerekurzivne metode računanja momenata $\mathcal{O}\left(\left(D - T + 1\right)\cdot T\right)$, odnosno približno $\mathcal{O}\left(DT\right)$, kako je $D \gg T$.
  ...
  Neka je $a\left[t\right]$ vremenski niz, za $t \in \left[0, 1, \ldots, D - 1\right]$.
  Nenormirani momenti $n$-tog reda $S_n$ niza $a\left[t\right]$ nad vremenskim prozorom duljine $T$ definirani su na sljedeći način:
  \begin{equation}
  S_n^{(t)} = \sum_{\tau=t-T+1}^{t} a^n\left[\tau\right].
  \end{equation}
  Za nulti moment uvijek vrijedi $S_0^{(t)} = T$ neovisno o $t$.
  Rekurzivni izraz za ažuriranje nenormiranog momenta je očigledan:
  \begin{align}
  S_n^{(t+1)} &= \sum_{\tau=t+1-T+1}^{t+1} a^n\left[\tau\right] \nonumber \\
    &= -a^n\left[t-T+1\right] + \sum_{\tau=t-T+1}^{t} a^n\left[\tau\right] + a^n\left[t+1\right] \nonumber \\
    &= -a^n\left[t-T+1\right] + S_n^{(t)} + a^n\left[t+1\right].
  \end{align}
  
  Preko nenormiranih momenata mogu se izraziti očekivanje i varijanca.
  Očekivanje $\Efromto{a\left[\tau\right]}{t - T < \tau \le t}$ je normirani moment prvog reda, i može se izraziti kao:
  \begin{equation}
  \Efromto{a\left[\tau\right]}{t - T < \tau \le t} = \frac{S_1^{(t)}}{S_0^{(t)}},
  \end{equation}
  ...
  Prema definiciji, varijanca $\Varfromto{a\left[\tau\right]}{t - T < \tau \le t}$ može se računati još i kao:
  \begin{align}
    \Varfromto{a\left[\tau\right]}{t - T < \tau \le t} &= \Efromto{\left(a\left[\tau\right] - \E{a\left[\tau\right]} \right)^2}{t - T < \tau \le t} \nonumber \\
    &= \Efromto{a^2\left[\tau\right] - 2 a\left[\tau\right] \E{a\left[\tau\right]} + \Esq{a\left[\tau\right]}}{t - T < \tau \le t} \nonumber \\
    &= \Efromto{a^2\left[\tau\right]}{t - T < \tau \le t} \nonumber \\
    & - 2\Esqfromto{a\left[\tau\right]}{t - T < \tau \le t} + \Esqfromto{a\left[\tau\right]}{t - T < \tau \le t} \nonumber \\
    &= \Efromto{a^2\left[\tau\right]}{t - T < \tau \le t} + \Esqfromto{a\left[\tau\right]}{t - T < \tau \le t}
  \end{align}
  Sada se $\Efromto{a^2\left[\tau\right]}{t - T < \tau \le t}$ također može izraziti preko nenormiranih momenata kao $\left. S_2^{(t)} \middle/ S_0^{(t)} \right.$.
  Konačno, varijanca se može izraziti preko nenormiranih momenata kao:
  \begin{equation}
    \Varfromto{a\left[\tau\right]}{t - T < \tau \le t} = \frac{S_2^{(t)}}{S_0^{(t)}} - \frac{\left(S_1^{(t)}\right)^2}{\left(S_0^{(t)}\right)^2}
    = \frac{S_0^{(t)} S_2^{(t)} - \left(S_1^{(t)}\right)^2}{\left(S_0^{(t)}\right)^2}.
  \end{equation}
  ...
  Inicijalno je potrebno izračunati momente $S_0^{(T - 1)}$, $S_1^{(T - 1)} $ i $S_2^{(T - 1)}$, te iz njih $\Efromto{a\left[\tau\right]}{0 \le \tau < T}$ i $\Varfromto{a\left[\tau\right]}{0 \le \tau < T}$; u složenosti $\mathcal{O}\left(T\right)$.
  Nadalje, za svaki pomak vremenskog prozora rekurzivno se ažuriraju vrijednosti nenormiranih momenata, te se putem njih izračunavaju očekivanje i varijanca, u složenosti $\mathcal{O}\left(T - D\right)$.
  Ukupna vremenska složenost algoritma time je pala s $\mathcal{O}\left(DT\right)$ na $\mathcal{O}\left(D\right)$.
