...
\chapter{Metode i koncepti}
  ...
  \begin{equation}
  \label{eq:statarb}
  V^{(0)} = 0, \quad P\left(V^{(t)} > 0\right) = 1,
  \end{equation}
  gdje $V^{(t)}$ označava vrijednost portfelja u trenutku $t$.
  ...
  Valja napomenuti da, iako nosi naziv arbitraža, statistička arbitraža zapravo ne zadovoljava nužno (\ref{eq:statarb}), jer korištene statističke mjere imaju određeni interval pouzdanosti manji od 1.
  Tako se može može dogoditi da bude $V^{(t)} < 0$ za neke $t$.
  Ipak, kako $t \to +\infty$, vjerojatnost $P\left(V^{(t)} > 0\right) \to 1$.
  U praksi, potrebno je osigurati dovoljno velik početni iznos $V^{(0)}$ kako bi se izbjegla mogućnost \textit{defaulta}.
  
  Algoritam statističke arbitraže najčešće čine sljedeći koraci (u svakom vremenskom trenutku):
  \begin{enumerate}
  ...
  Neka je sa $P_X^{(t)}$ označena log-cijena vrijednosnice $X$ u trenutku $t$, i neka se koristi period od $T$ vremenskih koraka.
  ...
  \begin{equation}
  \norm{P}_X^{(t)} = \frac{P_X^{(t)} - \Efromto{P_X^{(\tau)}}{t-T \le \tau < t}}{\sqrt{\Varfromto{P_X^{(\tau)}}{t-T \le \tau < t}}}.
  \end{equation}
  ...
  Nadalje, za svaki par vrijednosnica $X$ i $Y$ izračuna se razlika njihovih normaliziranih log-scijena $D_{X,Y}^{(t)}$ u trenutku $t$:
  \begin{equation}
  D_{X,Y}^{(t)} = \norm{P}_X^{(t)} - \norm{P}_Y^{(t)}
  \end{equation}
  Razlika normaliziranih log-cijena se koristi kao mjera sličnosti u algoritmu statističke arbitraže.
  Kod ove mjere normalizacijom se postiže ravnopravna usporedba dviju dionica različitih volatilnosti, što znači da će se prilikom usporedbe dviju dionica čiji su intenziteti promjene cijene različiti oni dovesti na istu razinu.
  
  \subsection{Normalizirana razlika cijena}
  Za razliku od prethodne mjere sličnosti, u ovom slučaju se razlika računa nad nenormaliziranim log-cijenama za sve $t$, označeno sa $d_{X,Y}^{(t)}$:
  \begin{equation}
  d_{X,Y}^{(t)} = P_X^{(t)} - P_Y^{(t)},
  \end{equation}
  a zatim se iz razlika $d_{X,Y}^{(t)}$ računaju normalizirane razlike $\norm{d}_{X,Y}^{(t)}$ za $t \ge T$:
  \begin{equation}
  \norm{d}_{X,Y}^{(t)} = \frac{d_{X,Y}^{(t)} - \Efromto{d_{X,Y}^{(\tau)}}{t-T \le \tau < t}}{\sqrt{\Varfromto{d_{X,Y}^{(\tau)}}{t-T \le \tau < t}}}.
  \end{equation}
