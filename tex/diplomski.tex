...
%  Organizacija poglavlja je sljedeća: \ldots
  ...
  \begin{description}
    \item[Vrijednosnica.] Vrijednosnica, ili vrijednosni papir \engl{financial asset} je 
    \item[Portfelj.] 
    \item[Likvidnost.]
    ...
    \item[Divizibilnost.]
    Divizibilnost je mogućnost da u portfelju budu prisutne količine vrijednosnica koje nisu cijeli brojevi.
    \item[Pozicija.]
    Pozicija označava stanje određene vrijednosnice koja je u portfelju.
    Pozicija može biti duga \engl{long}, kratka \engl{short} i neutralna \engl{neutral}.
    U dugoj poziciji, količina vrijednosnice u portfelju je pozitivna, odnosno investitor posjeduje određenu količinu kupljene vrijednosnice.
    Suprotno tome, u kratkoj poziciji, količina vrijednosnice u portfelju je negativna, što znači da je investitor prodao određenu količinu vrijednosnice koju nije posjedovao, i time se obavezao na kasniju kupnju iste količine.
    Na nekim tržištima nije dopuštena kratka pozicija u vrijednosnicama.
    Neutralna pozicija označava da se u portfelju ne nalazi ta vrijednosnica.
    Kada se govori o zatvaranju pozicije, radi se o prelasku u neutralnu poziciju.
    
    \item[Solventnost.] Solventnost označava da je vrijednost portfelja $V\q(t\w) \ge 0$ u svakom trenutku $t$. Portfelj koji zadovoljava svojstvo solventnosti je ostvariv portfelj; u suprotnom je neostvariv portfelj.
    \item[Indeks.]
...
%  w_{i,j}^{\q(t\w)} = \q. \q(c_{i,j}^{\q(t\w)} - m_{i,j}^{\q(t\w)}\w) \middle/ d_{i,j}^{\q(t\w)} \w.,
  ...
  Negativan predznak težine $w_{i,j}^{\q(t\w)}$ označava da je stvarni smjer preferencije obrnut od pretpostavljenog, odnosno brid koji ide od $j$ do $i$ s pozitivnom težinom $w_{j,i}^{\q(t\w)} = -w_{i,j}^{\q(t\w)}$ ekvivalentan je prvotno pretpostavljenom bridu.
...
%These findings suggest that the proposed method produces consistent returns and may be feasible in a live market setting.
  ...
  \begin{figure}[p]
    ...
    \caption{Rezultati simulacija trgovanja nad podskupom dionica S\&P 500 indeksa. Plavom bojom je prikazana krivulja kumulativnog profita bez uključenih troškova trgovanja, a zelenom uz troškove trgovanja od 0.10\%.}
    \label{fig:result1}
  \end{figure}
...
%  It adapts to the inconsistence of preferences by picking variable number of assets into the portfolio.
  ...
  Iako su istraženi specijalno financijski vremenski nizovi, matematički i statistički postupci koji su korišteni mogu potencijalno poslužiti i u druge svrhe.
  ...
  Although the research in this thesis has been done on financial time series, mathetmatical and statistical methods used can also prove useful in other applications.
