...
%  Organizacija poglavlja je sljedeća: \ldots
  ...
  \begin{description}
    \item[Vrijednosnica.] Vrijednosnica, ili vrijednosni papir \engl{security} je u širem smislu oblik imovine kojim se može trgovati.
  ...
  Teorem o nearbitraži također se može iskazati na malo složeniji način (prema \citep{ross2011elementary}), koji omogućuje bolje shvaćanje kada je arbitraža moguća, te kako se situacije u kojima nearbitražni uvjeti ne vrijede mogu iskoristiti.
  ...
  Neka je sa $r_i\q[j\w]$ označen iznos koji se dobije od jediničnog ulaganja u $i$-tu vrijednosnicu ako je ostvaren $j$-ti ishod.
  ...
  Za primarnu i dualnu formulaciju problema vrijedi načelo dualnosti koje tvrdi da, ako je bilo koji od ta dva problema riješiv, onda je maksimum primarnog jednak minimumu dualnog problema; no ako jedan od dvaju problema nije riješiv, onda za onaj drugi ne postoji jedinstveno rješenje.
  ...
  Neka je donja granica tih dobitaka označena sa $\rho$.
  ...
  Ograničenja jednakosti u dualnom problemu odgavaraju izrazu (\ref{eq:non-arbitrage}).
  Posljedica ovog odnosa dualnosti između izraza (\ref{eq:non-arbitrage}) i (\ref{eq:dual-arbitrage}) jest sljedeća: ako postoji vjerojatnosni vektor $\matr{p}$ koji ispunjava (\ref{eq:non-arbitrage}), tada je dualni problem riješiv i njegov minimum 0 je ispunjen, a prema načelu dualnosti to je ujedno i maksimum primarnog problema, $\rho = 0$.
...
%  In addition to ordering of the entities, utility function also provides the intensity of preference for a particular entity, hence it is more informative when it comes to the decision making.
  Funkcija korisnosti \engl{utility function} $U\colon \Omega \to \mathbb{R}$ je preslikavanje iz skupa dobara u skup realnih brojeva, tako da poredak preslikanih realnih brojeva odgovara poretku dobara prema individualnoj preferenciji, tj. vrijedi:
  \begin{equation*}
...
%  $\kappa$ ranges from 0 to 1, with 0 meaning full inconsistency (virtually unreachable), and 1 meaning full consistency.
  ...
  \end{equation*}
  Dobivena mjera konzistentnosti je manja od 1 što potvrđuje da početni graf nije bio intrinzično konzistentan.
...
%  Setting lower values for parameter $\alpha$ yields denser graphs, and setting $\alpha = 0$ always yields complete graphs.
  ...
  \end{equation}
  s bitnom razlikom da težine bridova postoje samo ako je prethodno zadovoljen prag $\beta$, a signal trgovanja je vremenski kontinuirana funkcija koja je podložna daljnjoj analizi.
  ...
  U područjima I i III signal je aktivan, odnosno potiče trgovanje, dok je u području II inaktivan, jer ne dopušta trgovanje.
  \begin{table}[h]
    \centering
  ...
  \begin{figure}[p]
    ...
    \caption{
      ...
      Između crtkanih linija nalazi se podruje u kojem je signal trgovanja aktivan. Uočljiva je sličnost u kretanju logaritamskih cijena prije i poslije aktivnog područja signala trgovanja.}
  ...
  \subsection{Selekcija parova odabirom praga trgovanja}
  \todo
