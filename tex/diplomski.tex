...
%These findings suggest that the proposed method produces consistent returns and may be feasible in a live market setting.
  ...
  \end{align}
  Sada se $\Efromto{a^2\q[\tau\w]}{t - T < \tau \le t}$ također može izraziti preko nenormiranih momenata kao $\q. S_2^{(t)} \middle/ S_0^{(t)} \w.$.
  ...
  Nadalje, za svaki pomak vremenskog prozora rekurzivno se ažuriraju vrijednosti nenormiranih momenata, te se putem njih izračunavaju očekivanje i varijanca, u složenosti $\mathcal{O}\q(T - D\w)$.
